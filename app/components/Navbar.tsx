...
const navLinks = [
  { label: "Workflow", href: "#how-it-works", section: "how-it-works" },
  { label: "Features", href: "#how-it-works", section: "how-it-works" },
...
const Navbar = () => {
  ...
  React.useEffect(() => {
    const sectionIds = [...new Set(navLinks.map((l) => l.section))];
  ...
  return (
    <>
      ...
      {dark && (
        <div className="fixed inset-0 z-0 pointer-events-none bg-[#0a0e1a]" />
      )}
      ...
      <div className="flex items-center gap-4">
        ...
        {isLoading ? (
            ...
            >
              {session.user?.image ? (
                <Image
                  ...
                  width={34}
                  height={34}
                  className="rounded-full border border-[#0F0F0F]/10"
                />
              ) : (
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-12 w-48 bg-white border border-[#0F0F0F]/[0.08] rounded-2xl shadow-lg py-2 z-50">
                ...
                <Link href="/dashboard" onClick={() => setDropdownOpen(false)}
                  className="flex items-center gap-2.5 px-4 py-2 text-sm text-[#0F0F0F] hover:bg-[#0F0F0F]/[0.04] transition-colors">
                  Dashboard
                </Link>
          ...
          <>
            <Link href="/login" className={`text-[0.82rem] hover:opacity-70 transition-colors ${dark ? "text-white" : "text-[#0F0F0F]"}`}>
              Login
            </Link>
            <Link href="/signup" className={`text-[0.82rem] hover:opacity-70 transition-colors ${dark ? "text-white" : "text-[#0F0F0F]"}`}>
              Sign up
            </Link>
            <Link href="/dashboard" className={`rounded-full px-5 py-2 text-[0.82rem] font-medium hover:opacity-80 transition-opacity duration-200 ${dark ? "bg-white text-[#0F0F0F]" : "bg-[#0F0F0F] text-white"}`}>
              Try Now
            </Link>
          </>
        )}
